...
len_each_row = list(map(len, input))[0]
num_elements = sum(list(map(len, input)))
print(len_each_row,num_elements)

# %%


def find_adjacents(x,length,max_elements):
    adjacents_list = [
        x-1, x+1, 
        (x-length),(x-length)+1, (x-length)-1,
        (x+length), (x+length)+1, (x+length)-1
    ]  
    # print(list(filter(lambda x : x>=0 and x<max_elements ,adjacents_list)))
    # print([x for x in adjacents_list if x>=0 and x<max_elements ] )
    return [x for x in adjacents_list if x>=0 and x<max_elements ] 
    #list(filter(lambda x : x>=0 and x<max_elements ,adjacents_list))

find_adjacents(0,len_each_row,num_elements)
find_adjacents(4,len_each_row,num_elements)

# %%
def isalnum_modified(c):
    """
    For numbers, strings, . -> return False, 
    for special chars -> return True
    """
    if c=="." or c=='\n':
        return False 
    else :
...
input_squished = "".join(input)
len(input_squished)

#%%
# 1. Find all groups of numbers

groups_all_numbers=re.finditer(r"\d+",input_squished)
# [[i.span(), i.group()] for i in groups_all_numbers ]
groups_all_numbers_start_end = [i.span() for i in groups_all_numbers ]
groups_all_numbers=re.finditer(r"\d+",input_squished)
all_numbers = list(map(int,[i.group() for i in groups_all_numbers ]))

#%%
remove_indices = []
remove_numbers = []
for start, end in groups_all_numbers_start_end : 
    #print(start,end)
    lst_alnum =[]
    for j in range(start, end):
        adj = find_adjacents(x = j, 
            length= len_each_row, 
            max_elements=num_elements)
    
        # if this list sum >0 -> then it has a special character adjacent to it, else no
        lst_alnum.append(
            sum(list(map(lambda y : isalnum_modified(y), [input_squished[k] for k in adj])))
        )
    print(lst_alnum,input_squished[start:end])
    if sum(lst_alnum)==0:
        remove_indices.extend(list(range(start, end)))
        remove_numbers.append(input_squished[start:end])

remove_numbers_int = list(map(int,remove_numbers))

sum(all_numbers)-sum(remove_numbers_int)
# %%
...
len(all_numbers), len(groups_all_numbers_start_end)
# %%

## Find all gears (*)

groups_stars=re.finditer(r"\*",input_squished)
groups_stars_start_end = [i.span() for i in groups_stars ]
groups_stars=re.finditer("\*",input_squished)
all_stars = [i.group() for i in groups_stars ]
all_stars
# %%
sum_gears=0
for start_coord_star,end_coord_star in groups_stars_start_end : 
    # start loc of *
    star_loc = start_coord_star
    loc_adjacents = find_adjacents(star_loc,len_each_row,num_elements)
    numbers_adjacent = 0
    number_adjacents_list = []
    for j in loc_adjacents : 
        for start_coord_no, end_coord_no in  groups_all_numbers_start_end:
            if j in range(start_coord_no, end_coord_no):
                number_adjacents_list.append((start_coord_no, end_coord_no))
        
    number_adjacents_list_remove_duplicates = list(set(number_adjacents_list))
    if len(number_adjacents_list_remove_duplicates)==2 :
        print(number_adjacents_list_remove_duplicates)
        mul=1
        for s,e in number_adjacents_list_remove_duplicates :
            mul*=int(input_squished[s:e])
        sum_gears+=mul
