...
for i in input:
    ...
    game = game_regex.search(i)
    game_ids.append(game.group(1))

    green_list = list(map(int,re.findall(green_reg,i))) 
    blue_list = list(map(int,re.findall(blue_reg,i))) 
    red_list = list(map(int,re.findall(red_reg,i))) 

    # lst_all = []
    # lst_all.append(green_list)
    # lst_all.append(blue_list)
    # lst_all.append(red_list)
    green_sum = sum(list(map(lambda x : x> green_max_lim,green_list)))
    blue_sum = sum(list(map(lambda x : x> blue_max_lim,blue_list)))
    red_sum = sum(list(map(lambda x : x>red_max_lim,red_list)))

    if green_sum==0 and blue_sum==0 and red_sum==0 : 
        valid_games.append(game.group(1))

    # # For each match, look-up corresponding value in dictionary
    # substituted_string = re.sub(r'Game \d+: ',"",i )

    # print(substituted_string.split(";"))

# %%

sum(list(map(int,valid_games)))
...
for i in input:
    ...
    sum_cubes+=max(green_list)*max(blue_list)*max(red_list)
